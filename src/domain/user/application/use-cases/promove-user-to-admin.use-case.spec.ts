...
import { PromoveUserToAdminUseCase } from './promove-user-to-admin.use-case'

let usersRepository: InMemoryUsersRepository
let sut: PromoveUserToAdminUseCase
describe('Promove user to admin', () => {
  beforeEach(() => {
    usersRepository = new InMemoryUsersRepository()
    sut = new PromoveUserToAdminUseCase(usersRepository)
  })
  ...
  it('should be able to promove a user to admin', async () => {
    const user = makeUser()
    ...
    expect(promovedUser?.role).toBe(Role.ADMIN)
  })

  it('should not be able, non admin user user, promote user to admin', async () => {
